...
def render():
    ...
    with t_bot:
        ...
        with c_bot3:
            ...
            ts_pct = st.number_input("Trailing Stop (%)", min_value=0.0, value=0.0, step=0.1, help="0.0 para desativar. Ex: 1.5% segue a posição a 1.5% do preço máximo.")
            ts_pct = st.number_input("Trailing Stop (%)", min_value=0.0, value=0.0, step=0.1, help="0.0 para desativar. Ex: 1.5% segue a posição a 1.5% do preço máximo.")
...
def _run_bot_cycle(place_order, close_position, get_positions):
    ...
    try:
        ...
        if active_pos:
            side = active_pos["side"].lower() # 'long' or 'short'
            entry = float(active_pos["avg_entry"])
            
            # Verificação de Stop Loss Fixo
            if sl_pct > 0.0:
            ...
            if ts_pct > 0.0:
                ...
                if extreme_price is None:
                    # Inicializa com o preço de entrada ou atual
            ...
            # Reseta o preço extremo se não houver posição ativa
            st.session_state.binance_bot_config["position_extreme_price"] = None

        # 2. Avaliar Sinais de Cruzamento da Estratégia
        signal = None
        ...
        if signal and signal != last_signal:
            try:
                if signal == "BUY":
                    ...
                    if active_pos:
                        ...
                        msg = f"[{now_str}] ℹ️ Sinal SELL mas sem posição aberta."
                st.session_state.binance_bot_log.append(msg)
            ...
            st.info(f"[{now_str}] 🔍 A monitorizar {symbol} - sem novo sinal (último: {last_signal})")
